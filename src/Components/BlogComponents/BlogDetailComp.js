...
const BlogDetailComp = ({data,thisuserallblog,previous,next,fun}) => {
    ...
    useEffect(()=>{
        if(data){
        setloading(true)
        Fire.child(`Comments/${data.User}/${data.BlogKey}`).on("value",function(snap){
    ...
    function Submit(e){
        ...
        Fire.child(`Comments/${data.User}/${data.BlogKey}`).push(obj,err=>{
        ...
        <div className="row gx-55 gx-5">
            <div className="col-lg-8">
                ...
                {previous?<a onClick={()=>{localStorage.setItem("BlogDetails",JSON.stringify(previous.BlogKey)); navigate("/BlogDetails"); fun(previous.BlogKey)}} className="prev-post">
                    ...
                    {next ?
                    <a  onClick={()=>{localStorage.setItem("BlogDetails",JSON.stringify(next.BlogKey)); navigate("/BlogDetails") ; fun(next.BlogKey)}}  className="next-post" >
                        <span>NEXT</span>
                    </a>:<a></a>}
                </div>
                 <div>
                <h3 className="comment-box-title">{comments?Object.keys(comments).length:"0"} Comments</h3>
                   ...
                   }
                {
                        ...
                        comments && Object.keys(comments)?.reverse()?.map((key,index)=> index<5 &&<div key={index} className={index%2===0?"comment-item":"comment-item reply"}>
                    ...
                    <form action="#" className="comment-form">
                        <div className="row">
                            ...
                            <div className="col-lg-12">
                                <div className="form-group">
                                    <textarea name="messages"  value={obj?.messages?obj?.messages:""} onChange={set}  id="messages" cols={30} rows={10} placeholder="Please Enter Your Comment Here" defaultValue={""} />
                                </div>
                            </div>
                            <div className="col-md-12 mt-3">
                                <button disabled={loading} onClick={Submit} className="btn-two">Post A Comment<i className="flaticon-right-arrow" /></button>
                            </div>
                        </div>
                    </form>
                </div>
